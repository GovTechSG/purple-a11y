...
const runAxeScan = async (needsReviewItems, includeScreenshots, page, customFlowDetails) => {
  const result = await runAxeScript(needsReviewItems, includeScreenshots, page, ${formatScriptStringVar(randomToken)}, customFlowDetails);
  ...
  const context = await browser.newContext({
    viewport: null,`);
      ...
      if (line.trim().includes('.fill(')) {
        ...
        appendToGeneratedScript(line);
        appendToGeneratedScript(`${locator}.focus();`);
