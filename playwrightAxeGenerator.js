...
import { proxy, getDefaultChromeDataDir } from './constants/constants.js';
...
const playwrightAxeGenerator = async data => {
  ...
  let { customDevice, viewportWidth } = data;
  ...
  const importStatements = `
    ...
    import { consoleLogger, silentLogger, guiInfoLog, guiInfoStatusTypes } from '#root/logs.js';
...
`;
  ...
  const customFlowScripts = './custom_flow_scripts';
  ...
  try {
    ...
    if (os.platform() === 'darwin') {
      browser = 'webkit';
      channel = '';
    } else if (!getDefaultChromeDataDir()) {
      // Windows
      channel = 'msedge';
    }
    ...
    const extraCodegenOpts = `${userAgentOpts} --browser ${browser} --block-service-workers --ignore-https-errors ${
    ...
    const codegenResult = execSync(codegenCmd, { cwd: dir });
    ...
    const appendToGeneratedScript = _data => {
      fs.appendFileSync(generatedScript, `${_data}\n`);
    };
