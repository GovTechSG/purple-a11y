...
import { deleteClonedProfiles, getBrowserToRun, getPlaywrightLaunchOptions, submitForm, urlWithoutAuth } from './constants/common.js';
...
export const init = async (entryUrl, testLabel, name = "Your Name", email = "email@domain.com", includeScreenshots = false, viewportSettings = { width: 1000, height: 660 }, // cypress' default viewport settings
thresholds = {}, scanAboutMetadata = undefined) => {
    ...
    const getScripts = () => {
        ...
        const axeScript = fs.readFileSync(path.join(__dirname, 'node_modules/axe-core/axe.min.js'), 'utf-8');
    ...
    const pushScanResults = async (res, metadata, elementsToClick) => {
        ...
        if (includeScreenshots) {
            ...
            const browserContext = await constants.launcher.launchPersistentContext(clonedBrowserDataDir, { viewport: scanAboutMetadata.viewport,
    ...
    const terminate = async () => {
        ...
        if (urlsCrawled.scanned.length === 0) {
            ...
            const basicFormHTMLSnippet = await generateArtifacts(randomToken, scanDetails.requestUrl, scanDetails.crawlType, null, scanDetails.urlsCrawled.scanned, pagesNotScanned, testLabel, scanAboutMetadata, scanDetails);
            await submitForm(constants.browserTypes.chromium, '', scanDetails.requestUrl, null, scanDetails.crawlType, email, name, JSON.stringify(basicFormHTMLSnippet), urlsCrawled.scanned.length, "{}");
        }
        return null;
    };
    return {
