...
import ejs from 'ejs';
import { createScreenshotsFolder, getStoragePath, getVersion, getWcagPassPercentage, formatDateTimeForMassScanner, retryFunction } from './utils.js';
...
const extractFileNames = async (directory) => fs
    .readdir(directory)
...
const writeResults = async (allissues, storagePath, jsonFilename = 'compiledResults') => {
    ...
    try {
        ...
        await fs.writeFile(`${storagePath}/reports/passed_items.json.txt`, JSON.stringify(passedItemsJson, null, 4));
    }
...
const writeCsv = async (allIssues, storagePath) => {
    ...
    const flattenRule = catAndRule => {
        ...
        const clausesArr = conformance.filter(clause => !['wcag2a', 'wcag2aa', 'wcag2aaa'].includes(clause));
...
export const generateArtifacts = async (randomToken, urlScanned, scanType, viewport, pagesScanned, pagesNotScanned, customFlowLabel, cypressScanAboutMetadata, scanDetails) => {
    ...
    const jsonArray = await Promise.all(allFiles.map(async (file) => parseContentToJson(`${directory}/${file}`)));
