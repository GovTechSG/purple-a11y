...
import { checkUrl, prepareData, isFileSitemap, validEmail, validName, getBrowserToRun, getPlaywrightDeviceDetailsObject, deleteClonedProfiles, getScreenToScan, getClonedProfilesWithRandomToken, validateDirPath, validateFilePath, validateCustomFlowLabel, } from './constants/common.js';
...
const options = yargs
    .version(false)
    .usage(`Purple A11y version: ${appVersion}
...
const scanInit = async (argvs) => {
    ...
    const res = await checkUrl(argvs.scanner, argvs.url, argvs.browserToRun, clonedDataDir, argvs.playwrightDeviceDetailsObject, isNewCustomFlow, argvs.header);
    switch (res.status) {
...
scanInit(options).then(async (storagePath) => {
    ...
    await fs
        ...
        if (process.env.REPORT_BREAKDOWN === '1') {
            messageToDisplay.push('Reports have been further broken down according to their respective impact level.');
        }
