...
import constants from '#root/constants/constants.js';
import { urlWithoutAuth } from '#root/constants/common.js';
import { getDefaultChromeDataDir } from './constants/constants.js';
...
const playwrightAxeGenerator = async (data) => {
    let { isHeadless, randomToken, deviceChosen, customDevice, viewportWidth, customFlowLabel, blacklistedPatternsFilename, includeScreenshots, } = data;
...
const runAxeScan = async (includeScreenshots, page, customFlowDetails) => {
  const result = await runAxeScript(includeScreenshots, page, ${formatScriptStringVar(randomToken)}, customFlowDetails);
    ...
    return urlsCrawled
        ...
        process.exit(1);
            ...
            const basicFormHTMLSnippet = await generateArtifacts(
              ...
              ${formatScriptStringVar(viewportWidth
...
`;
    ...
    try {
        ...
        let extraCodegenOpts = `${userAgentOpts} --browser ${browser} --block-service-workers --ignore-https-errors ${channel && `--channel ${channel}`}`;
        ...
        for await (let line of rl) {
            ...
            if (line === `const { chromium } = require('playwright');` ||
  ...
  const context = await browser.newContext({
    viewport: null,`);
            ...
            if (line === `const page = await context.newPage();`) {
                if (deviceChosen === 'Mobile') {
                    appendToGeneratedScript(line);
                    appendToGeneratedScript(`  const pageHeight = page.viewportSize().height
        ...
        `);
            ...
            if (line.includes(`.goto(`)) {
                ...
                if (!firstGoToUrl) {
                    ...
                    appendToGeneratedScript(`await page.goto(${formatScriptStringVar(firstGoToAddress)}, { timeout: 0 });`);
