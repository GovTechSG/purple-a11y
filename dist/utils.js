...
export const getUserDataFilePath = () => {
    ...
    if (platform === 'win32') {
        ...
        return path.join(process.env.HOME, 'Library', 'Application Support', 'Purple A11y', 'userData.txt');
    }
    else {
        // linux and other OS
...
export const getWcagPassPercentage = wcagViolations => {
    return parseFloat(((Object.keys(constants.wcagLinks).length - wcagViolations.length) /
...
export const zipResults = (zipName, resultsPath) => {
    ...
    if (os.platform() === 'win32') {
        try {
            execSync(`Get-ChildItem -Path "${resultsPath}\\*.*" -Recurse | Compress-Archive -DestinationPath "${zipName}"`, { shell: 'powershell.exe' });
        }
        catch (err) {
            throw err;
        }
