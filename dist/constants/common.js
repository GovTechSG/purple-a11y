...
import constants, { getDefaultChromeDataDir, getDefaultEdgeDataDir, getDefaultChromiumDataDir, proxy, formDataFields, } from './constants.js';
...
const checkUrlConnectivityWithBrowser = async (url, browserToRun, clonedDataDir, playwrightDeviceDetailsObject, isNewCustomFlow, extraHTTPHeaders) => {
    const res = {};
    let viewport = null;
    let userAgent = null;
...
export const checkUrl = async (scanner, url, browser, clonedDataDir, playwrightDeviceDetailsObject, isNewCustomFlow, extraHTTPHeaders) => {
    let res;
    if (proxy) {
        res = await checkUrlConnectivityWithBrowser(url, browser, clonedDataDir, playwrightDeviceDetailsObject, isNewCustomFlow, extraHTTPHeaders);
    }
    else {
...
export const prepareData = async (argv) => {
    ...
    const { scanner, headless, url, deviceChosen, customDevice, viewportWidth, playwrightDeviceDetailsObject, maxpages, strategy, isLocalSitemap, finalUrl, browserToRun, nameEmail, customFlowLabel, specifiedMaxConcurrency, fileTypes, blacklistedPatternsFilename, additional, metadata, followRobots, header, safeMode, } = argv;
...
export const getLinksFromSitemap = async (sitemapUrl, maxLinksCount, browser, userDataDirectory, userUrlInput, isIntelligent, username, password) => {
 ...
 */
export const getBrowserToRun = (preferredBrowser, isCli = false) => {
...
export const submitFormViaPlaywright = async (browserToRun, userDataDirectory, finalUrl) => {
    ...
    const browserContext = await constants.launcher.launchPersistentContext(clonedDir || userDataDirectory, {
        ...getPlaywrightLaunchOptions(browserToRun),
    });
    const page = await browserContext.newPage();
    try {
