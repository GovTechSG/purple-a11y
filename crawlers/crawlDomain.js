...
// import crawlIntelligentSitemap from './crawlIntelligentSitemap.js';
...
//   requestQueueFromIntelligent = null, //optional
//   pdfDownloadsFromIntelligent = null, //optional
//   uuidToPdfMappingFromIntelligent = null //optional
// ) => {
// let urlsCrawled
// let dataset;
// let requestQueue;
...
//   const sitemapLink = await findSitemap(url)
//   if (sitemapExist){
//     const urlsCrawled = await crawlIntelligentSitemap(
//       url,
//       sitemapLink,
//       randomToken,
//       host,
...
//   ({ dataset, requestQueue } = await createCrawleeSubFolders(randomToken));
//   pdfDownloads = [];
//   uuidToPdfMapping = {};
...
//   requestQueue=requestQueueFromIntelligent;
//   urlsCrawled = urlsCrawledFromIntelligent;
//   pdfDownloads = pdfDownloadsFromIntelligent;
//   uuidToPdfMapping = uuidToPdfMappingFromIntelligent
// }
...
//           if (isUrlPdf(req.url) || urlsCrawled.scanned.some(item => item.url === inputurl)) {
//             // playwright headless mode does not support navigation to pdf document
//             req.skipNavigation = true;
//           }
//           return req;
...
const crawlDomain = async (
  ...
  console.log('entered crawl Domain');
let dataset;
let urlsCrawled
let requestQueue;
...
try {
  ...
  console.log('error in crawlDomain :',error);
}
    ...
    requestHandler: async ({
      ...
      try {
        if (isBasicAuth) {
          ...
          if (isScanHtml) {
            // console.log('domain duplicate req? request.url :',request.url);
            const results = await runAxeScript(includeScreenshots, page, randomToken);
            guiInfoLog(guiInfoStatusTypes.SCANNED, {
              numScanned: urlsCrawled.scanned.length,
              urlScanned: request.url,
